...
$(() => {
    try {
        ...
        const restrictionMove = mw.config.get("wgRestrictionMove") || [];
        
        if (pageid === 0
            || $(".will2Be2Deleted")[0]
            || !mw.config.get("wgUserGroups").includes("patroller") && !mw.config.get("wgUserGroups").includes("sysop")
            || restrictionMove.includes("sysop") && !mw.config.get("wgUserRights").includes("editprotected")
            || restrictionMove.includes("techedit") && !mw.config.get("wgUserRights").includes("techedit")
