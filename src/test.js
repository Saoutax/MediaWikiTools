            ...
            async doMove() {
                ...
                try {
                    ...
                    if (typeof e.error?.code === "string") {
                        if (e.error.code === "moderation-move-queued") {
                            moveQueued = true;
                            console.log("[MoveToUserSubpage] Move queued for moderation");
                        } else {
                            throw e.error;
                        }
                    } else {
                ...
                if (!noNotice) {
                    ...
                    try {
                        ...
                        if (notifRes?.error) {
                            console.warn("[MoveToUserSubpage] Failed to post notification:", notifRes.error);
                        }
                    } catch (e) {
                        console.warn("[MoveToUserSubpage] Failed to post notification:", e);
                    }
                }

                if (moveQueued) {
                    console.log("[MoveToUserSubpage] Operation completed with move queued for moderation");
                }
            }
        }
